...
@UserActionMethod(command = {"stop", "play"}, buttonId = "stopButton", modalId = "stopModal")
public class StopAction implements UserAction {
    ...
    public Button Build(Guild guild, Message parent) {
        ...
        return style == ButtonStyle.UNKNOWN ? null : Button.of(style, "stopButton", label);
    }

    @Override
    public boolean OnClick(ButtonInteractionEvent event) {
        ActionManager.ExcuteMessageCommend(new MessageReceivedEvent(event.getJDA(), -1, event.getMessage()), "?stop", true);
